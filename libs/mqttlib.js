...
var mqttlib = new function () {
    ...
    this.init = function (ctx) {
        ...
        var clientId = 'nxtlevel_homebridge_' + config.name.replace(/[^\x20-\x7F]/g, "") + '_' + Math.random().toString(16).substr(2, 8);

        // start with any configured options object
        var options = config.mqttOptions || {};

        // standard options set by nxtlevel
        var myOptions = {
            keepalive: 10,
            clientId: clientId,
            protocolId: 'MQTT',
            ...
            will: {
                topic: 'WillMsg',
                payload: 'nxtlevel_homebridge [' + ctx.config.name + '] has stopped',
                qos: 0,
                retain: false
            },
